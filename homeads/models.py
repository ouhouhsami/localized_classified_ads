...
def geoad_new_interested_user_callback(sender, ad, interested_user, **kwargs):
    ...
    msg = NewPotentialBuyerToVendorMessageEmail(email_context, ad.user.email)
    msg.send()
...
def geoad_new_relevant_ad_for_search_callback(sender, ad, relevant_search, **kwargs):
    ...
    msg = NewAdToBuyerMessageEmail(email_context, relevant_search.ad_search.user.email)
    msg.send()
...
def geoad_user_message_callback(sender, ad, user, message, **kwargs):
    ...
    msg = BuyerToVendorMessageEmail(email_context, ad.user.email, user.email)
    msg.send()
...
def geoad_vendor_message_callback(sender, ad, user, message, **kwargs):
    ...
    msg = VendorToBuyerMessageEmail(email_context, ad.user.email, user.email)
    msg.send()
